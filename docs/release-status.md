...
- Selected source files were checked for private workspace paths and credential patterns. Model weights, run receipts, private session transcripts and full viewer video assets are excluded from the source archive.
...
The visual PDF and selected comparison PNGs are included in the repository. A cropped, labelled 24.5-second walkthrough is included; the full original recording remains separate. Source history stays in the clean project, with Jujutsu as the local VCS. Packaging does not start cloud generation or upload to a package index.
